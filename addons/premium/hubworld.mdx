...
<Note>
  The `World` field and the `Portals.List` serve different purposes. Setting `World` to a world name redirects **all** portal traffic through the hub world. If you set `World` to `'false'`, the `Portals.List` becomes active — players are teleported directly to a location from the list rather than to a hub world. Choose the approach that fits your server's design.
</Note>
